...
- **모델의 한계 섹션**과는 직접 연결되지 않음 — 환각·비결정성·토큰화는 모델 자체의 한계, 시스템 프롬프트는 *그 위에 깔리는 안전장치* 라는 별개의 레이어.
- **시스템 프롬프트 유출 사건 도입부** [데모: 02-system-prompt-leak.html]: 정확히 이 문서가 다루는 내용. 2026년 3월 유출 사건을 도입부로 쓰고, *"이 안전장치는 여러분이 Claude Code를 실행하는 순간 이미 깔려 있었다"* 라는 메시지로 1층(내장 하니스) 개념을 처음 소개.
- **1층 4요소 통합 슬라이드** [데모: 03-other-builtin.html]: 시스템 프롬프트 외에도 권한 체크·도구 경계·JSON 스키마 등 *다른 1층 요소들* 이 함께 등장. 이 문서는 그 중 첫 번째이자 가장 두꺼운 한 장에 해당.
- **1층+2층 통합 그림** [데모: 16-two-layers-combined.html]: "1층 = 시스템 프롬프트·권한체크·도구경계·JSON스키마" 4가지 통합 그림. 시스템 프롬프트는 그 중 *맨 앞에 깔리는 가장 큰 안전장치*.

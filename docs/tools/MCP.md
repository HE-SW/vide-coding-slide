...
```text
┌─────────────────────┐
│  Host (호스트)        │  사용자 AI 앱 (Claude Code, Cursor, ChatGPT, ...)
│                     │
│  ┌──────────┐       │
...
└──────┬──────────────┘
       ...
       ↓
┌─────────────────────┐
│  MCP Server         │  GitHub / Figma / Postgres / 직접 만든 것
│                     │
│  Tools (도구)       │  ← 모델이 호출 (예: list_issues, create_pr)
│  Resources (자원)   │  ← 모델이 *읽기*만 (예: 파일 내용, DB row)
│  Prompts (프롬프트) │  ← 재사용 가능 템플릿 (예: 보고서 양식)
└─────────────────────┘
```
...
| Primitive | 용도 | 예시 |
|---|---|---|
| **Tools** (도구) | 모델이 *호출하는 행동* | GitHub 이슈 생성, DB 쿼리 실행 |
| **Resources** (자원) | 모델이 *읽기만* 하는 데이터 | 파일 내용, 페이지 본문 |
| **Prompts** (프롬프트) | 재사용 가능한 템플릿 | 표준 보고서 양식, 코드 리뷰 체크리스트 |

**2가지 Transport**:
- **stdio** — 로컬 프로세스용 (가장 흔함). MCP 서버를 자식 프로세스로 띄워 표준입출력으로 통신.
- **Streamable HTTP** — 원격 서버용. 2025-11 명세에서 기존 SSE를 대체. 클라우드 서비스로 MCP 서버를 띄울 때 표준.
...
- **MCP·확장 도입 슬라이드** [데모: 23-mcp-extensions.html] — 정확히 이 문서의 도입부. *"이런 게 있다 정도만 알아두시면 된다"* 수준에서 짧게 언급. 이 문서는 그 *깊이 있는 보완*.
- **강의 전반**: MCP는 직접 실습엔 안 등장하지만, 강의자가 *"외부 세상과 어떻게 연결되는가"* 질문을 받았을 때 답할 수 있어야 한다.
...
- [Anthropic](../people/Anthropic.md) — MCP를 2024년 공개·2025년 Linux Foundation에 기증한 회사

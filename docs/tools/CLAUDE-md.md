...
> Claude Code가 매 세션 시작마다 자동으로 읽는 **프로젝트 사용설명서**. *"매번 같은 지시를 다시 입력하지 않아도 되게 만드는 한 장의 메모"* — 컨텍스트 엔지니어링의 가장 작은 실전 사례.
...
### (3-α) `CRITICAL:` 접두어 패턴 — *절대 위반 금지* 표시

규칙 중에서도 *"이건 어기면 큰일난다"* 수준의 항목은 줄 앞에 **`CRITICAL:`** 을 붙인다. `CLAUDE.md` 템플릿에서 자주 쓰는 작은 디테일.
...
- **모델은 *영어 대문자* 를 *강조 신호* 로 가중치 더 두는 경향**. Anthropic 공식 가이드도 `CRITICAL`·`IMPORTANT`·`NEVER` 같은 대문자 키워드를 *지시문 강도 표시* 로 권장.
- **bullet 한 줄 안의 위치도 중요** — 줄 *맨 앞* 에 둬야 *조건 절* 이 아니라 *전체 항목의 라벨* 로 읽힌다.

같은 계열로 자주 쓰이는 키워드:

| 키워드 | 강도 | 용도 |
|---|---|---|
| `CRITICAL:` | 최상 | 위반 시 프로젝트 무너짐 — TDD, 보안, 데이터 무결성 |
| `IMPORTANT:` | 상 | 어기면 품질·일관성 손상 |
| `NEVER` (대문자) | 상 | *절대* 하지 말 것 — `NEVER commit secrets` |
| `ALWAYS` (대문자) | 중상 | *항상* 따를 것 — `ALWAYS write tests first` |
| 일반 bullet | 중 | 권장 사항 |

남용 주의: *모든 줄을 `CRITICAL:`* 로 도배하면 — *전부 강조 = 전부 평이* 와 같은 효과가 나서 무게감이 사라진다. 한 `CLAUDE.md` 안에 `CRITICAL:` 은 *3~5개 이내* 가 적정선.
...
### (5) `@path` import 문법 — 한 장을 여러 조각으로

CLAUDE.md 가 길어지는 걸 방지하면서 *세부 규칙*은 따로 두는 패턴. 본문 안에 다음과 같이 적으면 그 파일을 자동으로 읽어들인다:
...
→ Claude Code가 세션 시작 시 *이 파일들도 함께* 컨텍스트에 주입. **CLAUDE.md는 짧게, 세부는 docs로** 패턴의 핵심 도구. *프로젝트 헌법(`CLAUDE.md`) → 프로젝트 뇌(`docs/`)* 로 자연스럽게 연결되는 매커니즘이다.

### (6) 4계층 메모리 — 더 넓은 그림

CLAUDE.md는 사실 Claude Code 메모리 시스템의 *한 종류*. 전체 메모리는 4계층:

| 계층 | 위치 | 범위 | 예시 |
|---|---|---|---|
| **Enterprise** | OS-level (`/Library/Application Support/...`) | 회사 전체 | 보안 규정 |
| **User** | `~/.claude/CLAUDE.md` | 내 모든 프로젝트 | 한국어 톤·개인 스타일 |
| **Project** | `./CLAUDE.md` | 이 프로젝트 | 기술 스택·컨벤션 |
| **Subdirectory** | `./src/feature-x/CLAUDE.md` | 폴더 단위 | 모듈별 특수 규칙 |

→ Claude는 **모두 합쳐서** 컨텍스트에 넣는다. 더 좁은 계층이 *우선* — Subdirectory > Project > User > Enterprise.
...
→ 강의 메시지 *"좋은 답변은 좋은 컨텍스트에서 나온다"* 의 **가장 손에 잡히는 첫 단계**.
...
- 슬라이드 파일은 `{강의폴더}/slides/NN-slug.html` 패턴
- 사실은 반드시 출처와 함께 적기
...
- [Claude Code 세션 운영](Claude-Code-세션운영.md) — 4계층 메모리·`@path` import의 *운영 측면*. `/clear` 후에도 살아남는 영속 컨텍스트의 위생

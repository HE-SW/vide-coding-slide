...
> Claude Code를 실행하는 *순간 자동으로 깔리는* 4가지 안전장치. 사용자가 손대지 않아도 동작하는 1층, 그 위에 우리가 2층(CLAUDE.md·docs·skills·hooks)을 올린다. **2강 #2~#4의 핵심 그림**.

## 1. 왜 "1층"이라고 부르나

- 2강에서 도입한 *2층 건물* 비유에서 가져온 말. **1층 = Claude Code 자체에 박혀 있는 하니스**, **2층 = 사용자가 추가하는 하니스**.
...
- **2강 #2** — 시스템 프롬프트(D 요소) 도입. 2026-03 유출이 도입부.
- **2강 #3** [데모: 03-other-builtin.html] — 나머지 3개(권한·도구·스키마)를 한 번에 소개. 이 문서가 정확히 그 페이지의 강의자 노트 역할.
- **2강 #4** [데모: 04-two-floor-diagram.html] — 1층 위에 2층을 올리는 *2층 건물* 그림. 이 문서가 1층 부분의 디테일.
- **2강 #16** [데모: 16-two-layers-combined.html] — 1층 4요소 + 2층 4요소 = 8개 통합 그림.

...
2. **대본 파일** (선택) — 상위 에피소드 폴더의 `script.md` 경로. 제공 시 [데모] 태그/주요 흐름에서 슬라이드 목록 자동 도출
3. **프레젠테이션 제목** — index.html 페이지 타이틀
4. **슬라이드 목록** — `{번호, slug, 제목}` 배열 (대본에서 자동 제안 가능)

사용자가 대본 파일만 제공한 경우, 대본을 파싱해서:
- [데모] 태그 위치 및 주요 전환점에서 슬라이드 후보 추출
...
타이틀에는 액센트 그라디언트(보라→하늘)를 일관되게 사용. 슬라이드를 섹션별로 색 구분하지 않으며, index.html도 단일 그리드로 출력한다.
...
핵심 규칙:
- 단일 4열 그리드 카드 레이아웃 (섹션 그룹화 없음)
- 모든 카드 동일한 스타일 (색상 구분 없음)
- nav 바 없음
- fade 전환 애니메이션 포함
...
2. **슬라이드 목록 확정** — 번호, slug, 제목, 레이아웃 타입을 표로 정리하고 사용자 확인
3. **index.html 먼저 생성** — F 섹션 템플릿 사용 (단일 그리드)
...
- [ ] index.html 단일 그리드, 섹션 그룹/색상 코딩 없음
...
7. **마지막 슬라이드(summary)**: glow orb 배경 + 액센트 그라디언트(보라→하늘) 사용 권장
